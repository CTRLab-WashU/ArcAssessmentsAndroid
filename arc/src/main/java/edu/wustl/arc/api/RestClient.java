...
import edu.wustl.arc.api.models.AuthDetailsRequest;
import edu.wustl.arc.api.models.CachedObject;
import edu.wustl.arc.api.models.DayProgress;
import edu.wustl.arc.api.models.DeviceRegistration;
import edu.wustl.arc.api.models.ExistingData;
import edu.wustl.arc.api.models.Heartbeat;
import edu.wustl.arc.api.models.CachedSignature;
import edu.wustl.arc.api.models.SessionInfo;
import edu.wustl.arc.api.models.SessionProgress;
import edu.wustl.arc.api.models.TestSchedule;
import edu.wustl.arc.api.models.TestScheduleSession;
...
import edu.wustl.arc.api.models.VerificationCodeRequest;
...
import edu.wustl.arc.study.CircadianRhythm;
...
import edu.wustl.arc.paths.home.HomeScreen;
            ...
            if(response.successful) {
                if (response.optional.has("contact_info")) {
                    ...
                    if (contactJson.has("phone")) {
                        ...
                        PreferencesManager.getInstance().putString(Study.TAG_CONTACT_INFO, Phrase.formatPhoneNumber(rawPhoneNumber));
                        success = true;
                    }

                    if(contactJson.has("email")) {
                        PreferencesManager.getInstance().putString(Study.TAG_CONTACT_EMAIL, contactJson.get("email").getAsString());
                        success = true;
                    }
                }
            }
    ...
    CallbackChain.Listener testScheduleListener = new CallbackChain.Listener() {
        ...
        public boolean onResponse(CallbackChain chain, RestResponse response) {
            if(response.successful){
                ...
                Hints.markShown(GridTutorial.HINT_PREVENT_TUTORIAL_CLOSE_GRIDS);
                Hints.markShown(PricesTutorial.HINT_PREVENT_TUTORIAL_CLOSE_PRICES);
                Hints.markShown(SymbolTutorial.HINT_PREVENT_TUTORIAL_CLOSE_SYMBOLS);

                Hints.markShown(TestInfoTemplate.HINT_GRID_TUTORIAL);
                Hints.markShown(TestInfoTemplate.HINT_PRICES_TUTORIAL);
                Hints.markShown(TestInfoTemplate.HINT_SYMBOL_TUTORIAL);
                Hints.markShown(TestInfoTemplate.HINT_REPEAT_TUTORIAL);

                Hints.markShown(HomeScreen.HINT_TOUR);
                Hints.markShown(HomeScreen.HINT_POST_BASELINE);
                Hints.markShown(HomeScreen.HINT_POST_PAID_TEST);
                Hints.markShown(HomeScreen.HINT_FIRST_TEST);
